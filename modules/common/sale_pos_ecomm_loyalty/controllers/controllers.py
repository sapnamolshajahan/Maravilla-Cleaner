...
class WebsiteSaleInherit(WebsiteSale):
	...
	@http.route(['/get-loyalty-points'], type='http', auth='public', website=True,sitemap=False)
	def get_loyalty_pts(self , **post):
		...
		web_currency = request.website.pricelist_id.currency_id
		if so:
	...
	@http.route(['/redeem-loyalty-points'], type='http', auth='public', website=True,sitemap=False)
	...
	def cart_update_json(self, product_id, line_id=None, add_qty=None, set_qty=None, **kw):
		...
		return super(WebsiteSaleInherit, self).cart_update_json(
	...
	def confirm_order(self, **post):
		...
		redirection = self._check_cart_and_addresses(order)
		if redirection:
			return redirection
		...
		today_date = datetime.today().date()
		loyalty_config = request.env['all.loyalty.setting'].sudo().search(
			[('active', '=', True), ('issue_date', '<=', today_date),
			 ('expiry_date', '>=', today_date)])
		is_loyalty_product = [line_id.product_id.id for line_id in order.order_line]
		for loyalty in loyalty_config:
			if loyalty.product_id.id not in is_loyalty_product:
...
class PortalLoyaltyHistory(CustomerPortal):
	...
	def _prepare_portal_layout_values(self):
		...
		values.update({
			'loyalty_count': partner.loyalty_pts,
		})
		return values	
	...
	def portal_my_wallet_transaction(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
		...
		values.update({
			...
			'points_total': str(partner.loyalty_pts),
			'pager': pager,
			'default_url': '/my/loyalty/history',
		})
		return request.render("sale_pos_ecomm_loyalty.portal_loyalty_history", values)
