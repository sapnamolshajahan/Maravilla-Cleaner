...
class res_partner(models.Model):
	...
	loyalty_pts = fields.Float(string='Loyalty Point',compute='_compute_loyalty_pts')
	...
	def _load_pos_data_fields(self, config_id):
		...
		params += ['loyalty_pts','loyalty_amount']
		return params
...
class all_loyalty_setting(models.Model):
	...
	product_id  = fields.Many2one('product.product','Product', domain = [('type', '=', 'service'),('available_in_pos','=',True),['invoice_policy','=','order']],required=True)
	issue_date  =  fields.Date(default=fields.Date.today(),required=True)
	expiry_date  = fields.Date('Expiry date',required=True)
	loyalty_basis_on = fields.Selection([('amount', 'Purchase Amount'), ('loyalty_category', 'Product Categories')],default='amount', string='Loyalty Basis On',required=True)
	...
	@api.model
	def _load_pos_data_fields(self, config_id):
		return ['id', 'name', 'product_id', 'issue_date', 'expiry_date', 'loyalty_basis_on', 'active', 'loyality_amount','redeem_ids','multi_company_ids','company_id']
...
class web_redeem_rule(models.Model):
	...
	@api.model
	def _load_pos_data_fields(self, config_id):
		return ['name','min_amt','max_amt','reward_amt','loyality_id']
...
class web_loyalty_history(models.Model):
	...
	generated_from = fields.Selection([('sale', 'Sale'), ('pos', 'POS'),('website','Website'), ('prezzy', 'Issued Prezzy Card'), ('other', 'Other')], string='Generated From')
