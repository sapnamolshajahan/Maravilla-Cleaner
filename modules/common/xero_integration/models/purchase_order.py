    ...
    def exportPurchaseOrder(self):
        ...
        for t in purchase:
            if not t.xero_purchase_id:
                ...
                if xero_token.xero_oauth_token:
                    token = xero_token.xero_oauth_token
                ...
                if token:
                    ...
                    if data.status_code == 200:
                        self.env['xero.error.log'].success_log(record=t, name='Purchase Order Export')
                        ...
                        self.env['xero.error.log'].error_log(record=t, name='Purchase Order Export', error=data.text)
                        self._cr.commit()
                        response_data = json.loads(data.text)
                        if response_data:
